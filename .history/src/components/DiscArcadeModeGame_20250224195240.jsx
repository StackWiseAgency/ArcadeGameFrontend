...
const DiscArcadeModeGame = ({ navigateToSelection }) => {
  ...
  const API_RECEIVE_MOVE = "https://arcadegamebackendapi20241227164011.azurewebsites.net/api/Webhook/ImpinjRFIDReaderHook";
  ...
  useEffect(() => {
    if (!useApiInput || !gameStarted) return; // ✅ Ensure API input is enabled and game is running
      ...
      try {
        ...
        if (response.status === 200 && response.data.tagReads) {
          response.data.tagReads.forEach(({ epctag, row, col }) => {
            ...
            handleInputThrow(epctag, row, col); // ✅ Pass move to game logic
          });
        }
      } catch (error) {
    ...
    // ✅ Poll the API every 2 seconds for new moves
