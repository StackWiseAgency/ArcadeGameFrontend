...
const PlayersQueue = ({ onClose }) => {
  ...
  const authenticateUser = async () => {
    ...
    try {
      ...
      if (response.data && response.data.dataModel && response.data.dataModel.user) {
        const loggedInUserId = response.data.dataModel.user.id; // ✅ Extract correct userId
  ...
  // ✅ Start Game (Only for the first player in the queue)
  let gameWindow = null;
  const startGameSession = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Convert game name for URL navigation
      ...
      // ✅ Open the game screen in a fixed URL only once
      // const gameScreenUrl = "http://localhost:3000/game-screen"; // Replace with your static URL
      // if (!gameWindow || gameWindow.closed) {
      //   gameWindow = window.open(gameScreenUrl, "_blank");
      // } else {
      //   gameWindow.focus(); // Bring the existing tab to front
      // }
      const gameScreenUrl = "http://localhost:3000/game-screen"; // Static URL
      if (!gameWindow || gameWindow.closed) {
        gameWindow = window.open(gameScreenUrl, "_blank");
      } else {
        gameWindow.focus(); // Bring the existing tab to the front
      }
      } else {
