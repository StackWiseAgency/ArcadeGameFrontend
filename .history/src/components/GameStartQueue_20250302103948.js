...
const PlayersQueue = ({ onClose }) => {
  ...
  const startGameSession = async () => {
    ...
    try {
      ...
      //   if (firstPlayer.userId !== null && firstPlayer.userId !== 0) {
      //     apiUrl += `?userId=${firstPlayer.userId}`;
      // } else if (firstPlayer.guestId !== null && firstPlayer.guestId !== 0) {
      //     apiUrl += `?guestId=${firstPlayer.guestId}`;
      // }
      ...
      // const response = await axios.post(apiUrl);
          ...
          if (firstPlayer.userId !== null && firstPlayer.userId !== 0) {
            apiUrl += `?id=${firstPlayer.userId}`;  // ✅ Using "id" instead of "userId"
        } else if (firstPlayer.guestId !== null && firstPlayer.guestId !== 0) {
            apiUrl += `?id=${firstPlayer.guestId}`;  // ✅ Using "id" instead of "guestId"
        }
        
        console.log("🚀 API Request URL:", apiUrl);

    //   const requestData = {
    //     userId: firstPlayer.userId && firstPlayer.userId !== 0 ? firstPlayer.userId : null,
    //     guestId: !firstPlayer.userId || firstPlayer.userId === 0 ? firstPlayer.guestId : null
    // };

    // console.log("🚀 API Request Payload:", requestData);

    // // Send JSON request instead of using query parameters
    // const response = await axios.post(API_START_GAME, requestData, {
    //     headers: {
