...
const CheckoutPage = () => {
  ...
  const handleCardPayment = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/payment/create-payment-intent`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount, currency: "usd", email: "test@example.com" }),
      });
  ...
  const generateCheckoutQR = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/payment/create-checkout-session`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="checkout-container">
      ...
      {paymentStatus === "qrReady" && checkoutUrl && (
        <div className="qr-container">
          ...
          <p><a href={checkoutUrl} target="_blank" rel="noopener noreferrer">Click Here</a> to pay.</p>
        </div>
      )}
    </div>
  );
