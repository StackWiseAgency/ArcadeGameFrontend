...
import TicTacClash from "../assets/tic-tac-toe.png";
import LightsOut from "../assets/lights-out.png";
import DiscArcade from "../assets/retro-golf.png";
...
const gameImages = {
  "Disc Golf": DiscGolf,
  "Tic Tac Clash": TicTacClash,
  "Lights Out": LightsOut,
  "Disc Arcade": DiscArcade,
};
useEffect(() => {
  const fetchGames = async () => {
    try {
      ...
      const fetchedGames = result.dataModel.map(game => ({
        ...
        image: gameImages[game.name] || DiscGolf,
      }));

      setGames(fetchedGames);

      // Store the id and name of all received games in localStorage
      if (result.dataModel && result.dataModel.length > 0) {
        const gamesData = result.dataModel.map(game => ({
          id: game.id,
          name: game.name
        }));
        // Store the array of games in localStorage as a JSON string
        localStorage.setItem("allGames", JSON.stringify(gamesData));
      }
...
// useEffect(() => {
  ...
  const handlePlayClick = (game) => {
    ...
    //   setIsModalOpen(true);
    // }
    localStorage.setItem("selectedGame", JSON.stringify({ id: game.id, name: game.name }));
  };
              ...
              <div className="game-card" key={index}>
                ...
                <div className="game-info">
                  ...
                  >
                   ...
                   {/* If it's the 2nd or last game, show "Coming Soon" on button and disable it */}
              {(index === 1 || index === games.length - 1) ? (
                <button
