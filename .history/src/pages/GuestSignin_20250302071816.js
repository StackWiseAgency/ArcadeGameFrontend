  ...
  const handleContinue = async () => {
    if (guestName) {
      try {
        ...
        if (response.status === 200 && response.data && response.data.dataModel) {
          const guestId = response.data.dataModel.id; // Extract guestId from response

          // Save guestName & guestId to local storage
