...
const CustomKeyboard = ({ onChange, buttonLabel = "Keyboard" }) => {
  ...
  const initialTop = 200; // Default initial top position (can be adjusted)
  const initialLeft = 700; // Default initial left position (can be adjusted)
  ...
  const toggleKeyboardVisibility = () => {
    ...
    if (!keyboardVisible) {
      ...
      setPosition({ x: initialTop, y: initialLeft, zIndex: initialZIndex}); // Adjust these values as per your requirement
    }
  };
